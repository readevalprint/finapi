...
> IdentifierList delete_all_transactions(max_deletion_date=max_deletion_date, safe_mode=safe_mode, remember_deletion=remember_deletion)
...
remember_deletion = false # bool | When passing 'true', then finAPI will make sure to not re-import deleted transactions on future account updates. When 'false', then deleted transactions might be re-imported. Default value for this parameter is 'false'. (optional) (default to false)

try:
    # Delete all transactions
    api_response = api_instance.delete_all_transactions(max_deletion_date=max_deletion_date, safe_mode=safe_mode, remember_deletion=remember_deletion)
    pprint(api_response)
...
------------- | ------------- | ------------- | -------------
 **max_deletion_date** | **str**| If specified, then only those transactions are being deleted whose &#39;finapiBookingDate&#39; is equal to or earlier to the given date. The date may not be in future, and must be given in the format &#39;YYYY-MM-DD&#39;. If not specified, then no date limitation will be in place for the deletion. | [optional] 
...
# create an instance of the API class
api_instance = finapi.TransactionsApi(finapi.ApiClient(configuration))
body = finapi.UpdateMultipleTransactionsParams() # UpdateMultipleTransactionsParams | Update transactions parameters
...
body = finapi.UpdateTransactionsParams() # UpdateTransactionsParams | Update transactions parameters
...
purpose = 'purpose_example' # str | If specified, then only those transactions will be contained in the result whose purpose field contains the given search string (the matching works case-insensitive). If no transactions contain the search string in the purpose field, then the result will be an empty list. NOTE: If the given search string consists of several terms (separated by whitespace), then ALL of these terms must be contained in the purpose in order for a transaction to get included into the result. (optional)
...
try:
    # Get and search all transactions
    api_response = api_instance.get_and_search_all_transactions(view, ids=ids, search=search, counterpart=counterpart, purpose=purpose, account_ids=account_ids, min_bank_booking_date=min_bank_booking_date, max_bank_booking_date=max_bank_booking_date, min_finapi_booking_date=min_finapi_booking_date, max_finapi_booking_date=max_finapi_booking_date, min_amount=min_amount, max_amount=max_amount, direction=direction, label_ids=label_ids, category_ids=category_ids, include_child_categories=include_child_categories, is_new=is_new, is_potential_duplicate=is_potential_duplicate, is_adjusting_entry=is_adjusting_entry, min_import_date=min_import_date, max_import_date=max_import_date, page=page, per_page=per_page, order=order)
    pprint(api_response)
except ApiException as e:
 ...
 **purpose** | **str**| If specified, then only those transactions will be contained in the result whose purpose field contains the given search string (the matching works case-insensitive). If no transactions contain the search string in the purpose field, then the result will be an empty list. NOTE: If the given search string consists of several terms (separated by whitespace), then ALL of these terms must be contained in the purpose in order for a transaction to get included into the result. | [optional] 
...
api_instance = finapi.TransactionsApi(finapi.ApiClient(configuration))
id = 789 # int | Transaction identifier
body = finapi.SplitTransactionsParams() # SplitTransactionsParams | Split transactions parameters
...
# create an instance of the API class
api_instance = finapi.TransactionsApi(finapi.ApiClient(configuration))
body = finapi.TriggerCategorizationParams() # TriggerCategorizationParams | Trigger categorization parameters
